...
def load_model_and_preprocessor(model_dir="ml_models"):
    ...
    model_path = Path(model_dir) / "best_model_IE_model.pkl"
...
def predict_single(model, preprocessor, conditions):
    ...
    # Encode acid type: H2SO4=0, HCl=1
    conditions["acid_type_encoded"] = (
        1 if conditions.get("acid", "H2SO4") == "HCl" else 0
    )
...
def predict_batch(model, preprocessor, input_csv, output_csv=None):
    ...
    df["temp_conc_interaction"] = (
        df["temperature_C"] * df["inhibitor_conc_mg_L"] / 1000.0
    )
    df["acid_strength_norm"] = df["acid_molarity_M"] / 0.5
    df["acid_type_encoded"] = df["acid"].apply(lambda x: 1 if x == "HCl" else 0)
...
def generate_concentration_response(
    model, preprocessor, conditions, conc_range=None, output_path=None
):
    """
    ...
    df_response = pd.DataFrame(
        {"concentration_mg_L": conc_range, "predicted_IE_pct": predictions}
    )
    ...
    if output_path:
        ...
        ax.fill_between(
            ...
            alpha=0.2,
            color="blue",
            label=f"±{uncertainty}% uncertainty",
        )
        ...
        ax.set_title(
            f'Concentration-Response Curve: {conditions.get("inhibitor_name", "Unknown")}',
            fontweight="bold",
            fontsize=14,
        )
...
def interactive_prediction():
    ...
    print("(Trained on: Curry leaf extract, Peanut shell extract, Aloe vera extract)")

    conditions = {}
    conditions["inhibitor_name"] = (
        input("  Inhibitor name [default: Curry leaf extract]: ")
        or "Curry leaf extract"
    )
    conditions["inhibitor_conc_mg_L"] = float(input("  Concentration (mg/L): "))
    conditions["temperature_C"] = float(
        input("  Temperature (°C) [default: 25]: ") or "25"
    )
    conditions["acid"] = (
        input("  Acid type (H2SO4 or HCl) [default: H2SO4]: ") or "H2SO4"
    )
    conditions["acid_molarity_M"] = float(
        input("  Acid molarity (M) [default: 1.0]: ") or "1.0"
    )
    conditions["immersion_time_h"] = float(
        input("  Immersion time (hours) [default: 1]: ") or "1"
    )
    ...
    print(f"Acid: {conditions['acid_molarity_M']}M {conditions['acid']}")
...
def main():
    ...
    parser.add_argument("--inhibitor", help="Inhibitor name for response curve")
    ...
    if args.mode == "interactive":
        ...
        output_path = (
            args.output or f"response_curve_{args.inhibitor.replace(' ', '_')}.png"
        )
